...
$phoen_apply_box_notification_checkout_page = isset($this->phoen_rewpts_notification_data['phoen_apply_box_notification_checkout_page']) ? $this->phoen_rewpts_notification_data['phoen_apply_box_notification_checkout_page'] : 'You can apply {points} Points to get {price} Discount.';
...
if($total_reward_point > 0 && !empty($total_reward_point)):
	...
	$phoen_apply_box_notification_checkout_page = str_replace("{points}","$total_reward_point",$phoen_apply_box_notification_checkout_page);
					
	$total_reward_point = str_replace("{price}","$current_currency_symbol$total_reward_amount",$phoen_apply_box_notification_checkout_page); 
	...
	if(!in_array('reward amount',$woocommerce->cart->get_applied_coupons())):  ?>
		...
		<div style="padding: 1%;font-size: 100%;" class='woocommerce-cart-notice woocommerce-cart-notice-minimum-amount woocommerce-info'>
			...
			<div class='phoen_rewpts_pts_link_div cart_apply_btn' style="width:50%;">
				...
				<form method='post' action=''>
					...
					<input style="font-size: 100%;background: <?= _e($button_bg_color)?>;color: <?= _e($button_text_color)?>;" type='submit' class='button primary'  value='<?= _e($apply_btn_title)?>' name='apply_points'>
